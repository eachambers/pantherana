...
# TODO fix below; also unclear why this script generates a X_indv.txt file when
# this shouldn't be dependent on the amounts of missing data
##    FILES REQUIRED:
##            X.lmiss
...
# Specify dataset
lmiss_prefix = "spp_delim/foothills_06" # forreri_0.6miss_ldp / foothills_06 / mxpl_06
...
# Check inds
imiss <- read.table(paste0(lmiss_prefix, ".imiss"), header = TRUE) %>% 
  dplyr::select(IID) %>% 
  rename(Bioinformatics_ID = IID)
...
if (dataset_name == "foothills") {
  metadata <- read_tsv(paste0(here("data"), "/PACMX_metadata.txt"), col_names = TRUE)
  inds <- read_tsv("spp_delim/foothills_indv.txt", col_names = FALSE) %>% 
    rename(Bioinformatics_ID = X1)
}

if (dataset_name == "forreri") metadata <- read_tsv(paste0(here("data"), "/forreri_metadata.txt"), col_names = TRUE)

if (dataset_name == "mxpl") {
  metadata <- read_tsv(paste0(here("data"), "/ATL_MXPL_metadata.txt"), col_names = TRUE)
  inds <- read_tsv("spp_delim/mxpl_indv.txt", col_names = FALSE) %>% 
    rename(Bioinformatics_ID = X1)
}
...
# write_tsv(dat %>% dplyr::select(Bioinformatics_ID), paste0("spp_delim/", dataset_name, "_indv.txt"), col_names = FALSE) # list to keep

# List to remove
toremove <- setdiff(metadata %>% dplyr::select(Bioinformatics_ID),
                    inds) # lapply(function(x) paste("^", x, sep = ""))

write_tsv(toremove, paste0("spp_delim/", dataset_name, "_remove.txt"), col_names = FALSE)
